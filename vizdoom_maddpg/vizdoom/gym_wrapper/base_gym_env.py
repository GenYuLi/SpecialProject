...
class VizdoomEnv(gym.Env):
    def __init__(
        ...
        if(agent_num == 2):
            ...
            self.game.set_ticrate(1.5 * vzd.DEFAULT_TICRATE)
    ...
    def step(self, action):
        ...
        # 將返回的Agent觀察資料、獎勵、結束訊號、除錯資訊更改為list型態
        # 以符合該MADDPG架構
        observations = []
        # 觀測資料目前僅回傳畫面資訊(RGB24)
        observations.append(self.__collect_observations()['screen'])
        rewards = []
    ...
    def reset(
        ...
        *,
        ...
        # 觀測資料目前僅回傳畫面資訊(RGB24)
        if not return_info:
            return self.__collect_observations()['screen']
        else:
            return self.__collect_observations()['screen'], {}
